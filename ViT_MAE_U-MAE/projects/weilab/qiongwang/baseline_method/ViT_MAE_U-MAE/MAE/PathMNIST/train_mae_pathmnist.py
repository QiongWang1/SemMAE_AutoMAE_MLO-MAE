"""
MAE (Masked Autoencoder) Baseline for PathMNIST Classification
...
class MAE_Encoder(nn.Module):
    ...
    def forward(self, x, ids_keep=None):
        ...
        if ids_keep is not None:
            # Only for visible patches during pretraining
            pos_embed_visible = self.pos_embed[:, ids_keep, :]
            x = x + pos_embed_visible
        else:
            # All patches during finetuning
            x = x + self.pos_embed
...
class MAE_Decoder(nn.Module):
    """MAE Decoder (reconstructs all patches from visible patches)"""
    ...
    def forward(self, x, ids_restore):
        ...
        x_full = torch.gather(x, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))
...
def main():
    parser = argparse.ArgumentParser(description="MAE Baseline for PathMNIST")
