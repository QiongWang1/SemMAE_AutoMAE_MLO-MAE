"""
DermaMNIST DataLoader for SemMAE
Adapts DermaMNIST dataset for MAE-style training
"""
import torch
...
class DermaMNISTDataset(Dataset):
    """
    DermaMNIST dataset wrapper for MAE pretraining
    Input: DermaMNIST 28x28 images
    ...
    def __init__(self, medmnist_dataset, transform=None, target_size=32):
        """
        Args:
            medmnist_dataset: The DermaMNIST dataset object
    ...
    def __getitem__(self, idx):
        # Get image and label from DermaMNIST
...
def build_derma_dataset(split='train', download=True, transform=None, target_size=32):
    """
    ...
    medmnist_data = DermaMNIST(split=split, download=download)
    
    # Wrap with our custom dataset
    dataset = DermaMNISTDataset(medmnist_data, transform=transform, target_size=target_size)
