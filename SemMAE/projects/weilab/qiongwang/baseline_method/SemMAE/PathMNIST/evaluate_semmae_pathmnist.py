"""
Evaluation script for SemMAE on PathMNIST
...
# Add parent directory to path to access SemMAE modules
...
class LinearClassifier(nn.Module):
    ...
    def __init__(self, encoder, num_classes=9, embed_dim=192):
        super().__init__()
...
def train_linear_classifier(encoder, train_loader, val_loader, device, num_epochs=50, lr=0.001, embed_dim=192):
    ...
    model = LinearClassifier(encoder, num_classes=9, embed_dim=embed_dim).to(device)
...
def main():
    parser = argparse.ArgumentParser('Evaluate SemMAE on PathMNIST')
    ...
    train_dataset = build_pathmnist_dataset(split='train', download=True, transform=transform, target_size=args.input_size)
    val_dataset = build_pathmnist_dataset(split='val', download=True, transform=transform, target_size=args.input_size)
    test_dataset = build_pathmnist_dataset(split='test', download=True, transform=transform, target_size=args.input_size)
    ...
    output_file = results_dir / 'SemMAE_PathMNIST_Results.md'
    
    with open(output_file, 'w') as f:
        f.write("# SemMAE on PathMNIST - Evaluation Results\n\n")
        ...
        f.write("- **Source:** MedMNIST PathMNIST\n")
        f.write("- **Task:** Multi-class classification (9 classes)\n")
        f.write("- **Domain:** Colorectal cancer histopathology images\n\n")
    ...
    np.save(preds_dir / 'test_predictions.npy', np.array(results['predictions']))
    np.save(preds_dir / 'test_labels.npy', np.array(results['labels']))
    np.save(preds_dir / 'test_probabilities.npy', np.array(results['probabilities']))
    ...
    metrics_dict = {
        ...
        'f1': float(results['f1']),
        'confusion_matrix': results['conf_matrix'].tolist(),
        'dataset_info': {
