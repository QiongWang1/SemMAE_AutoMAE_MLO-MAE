...
parser.add_argument('--wandb_project', type=str, default='PathMNIST-v2-MLO-MAE-Finetune', help='wandb project name')
parser.add_argument('--wandb_run_name', type=str, default='pathmnist_v2_finetune', help='wandb run name')
...
if usewandb:
    import wandb
    watermark = args.wandb_run_name if hasattr(args, 'wandb_run_name') else "{}_lr{}_pathmnist_v2".format(args.net, args.lr)
    wandb.init(project=args.wandb_project if hasattr(args, 'wandb_project') else "PathMNIST-v2-MLO-MAE",
            name=watermark)
...
def log_prediction_images(net, dataloader, device, epoch, num_images=16):
    ...
    # Denormalize images
    mean = torch.tensor(PATH_MEAN).view(1, 3, 1, 1)
    std = torch.tensor(PATH_STD).view(1, 3, 1, 1)
...
if args.use_pretrained:
    pretrain_path = os.path.join(args.checkpoint_dir, 'pretrain_best.pth')
...
if args.use_finetune:
    finetune_path = os.path.join(args.checkpoint_dir, 'finetune_best.pth')
...
if args.net=='mlomae':
    ...
    raise ValueError(f"Model {args.net} not supported for PathMNIST")
