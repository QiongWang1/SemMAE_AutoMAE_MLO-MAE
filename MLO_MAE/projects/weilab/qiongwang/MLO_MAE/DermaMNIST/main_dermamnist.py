...
def save_checkpoint(epoch, model, optimizer, output_directory="./checkpoint_dermamnist", filename="checkpoint.pth.tar"):
...
path_save = './checkpoint_dermamnist'
...
def get_args_parser():
    parser = argparse.ArgumentParser('MLO-MAE DermaMNIST pre-training', add_help=False)
    ...
    parser.add_argument('--output_dir', default='./ckpt_dermamnist/test',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./log_dermamnist/test',
    ...
    # additional setting
    parser.add_argument('--wandb_project', type=str, default='MLO_MAE_DermaMNIST')
    parser.add_argument('--wandb_run_name', type=str, default='dermamnist_mlo_pretrain')
...
def main(args):
    ...
    # DermaMNIST normalization - use ImageNet normalization for better transfer learning
    ...
    train_data = DermaMNIST(split='train', transform=train_transform, download=True, root=args.data_path)
    valid_data = DermaMNIST(split='val', transform=valid_transform, download=True, root=args.data_path)
    test_data = DermaMNIST(split='test', transform=valid_transform, download=True, root=args.data_path)
...
#        drop_last=True,
    ...
    finetune_module = models_vit_mlo_ddp.FinetuneVisionTransformer(num_classes=7).to(device)
        ...
        def validation(self):
            ...
            if not args.baseline:
                ...
                if best_acc < acc:
                    ...
                    save_checkpoint(None, model, None, output_directory="./checkpoint_dermamnist", filename="pretrain.pth")
                    save_checkpoint(None, finetune_module, None, output_directory="./checkpoint_dermamnist", filename="finetune.pth")
                    save_checkpoint(None, masking_module, None, output_directory="./checkpoint_dermamnist", filename="masking.pth")
            else:
