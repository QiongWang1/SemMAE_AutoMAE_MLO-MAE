...
# parsers
parser = argparse.ArgumentParser(description='PyTorch PathMNIST Fine-tuning')
...
parser.add_argument('--checkpoint_dir', default='./checkpoint_pathmnist', type=str, 
                    help='Directory to save best model checkpoint (finetune_best.pth)')
parser.add_argument('--output_dir', default=None, type=str,
                    help='Directory for intermediate checkpoints and outputs (defaults to checkpoint_dir)')
parser.add_argument('--pretrained_path', default='./checkpoint_pathmnist', type=str,
                    help='Directory containing pretrained encoder (pretrain.pth)')
parser.add_argument('--data_path', default='./data', type=str)
parser.add_argument('--wandb_project', type=str, default='PathMNIST-MLO-MAE-Finetune', help='wandb project name')
parser.add_argument('--wandb_run_name', type=str, default='pathmnist_finetune', help='wandb run name')

args = parser.parse_args()

# Handle environment variable for OUT_ROOT (SLURM job-specific output)
if os.getenv('OUT_ROOT'):
    out_root = os.getenv('OUT_ROOT')
    print(f"Using OUT_ROOT from environment: {out_root}")
    # Override paths if OUT_ROOT is set
    if not args.output_dir:
        args.output_dir = os.path.join(out_root, 'checkpoint')
    if args.checkpoint_dir == './checkpoint_pathmnist':  # Only override if default
        args.checkpoint_dir = os.path.join(out_root, 'best_checkpoint')

# Set output_dir to checkpoint_dir if not specified
...
print(f"  Pretrained models:     {args.pretrained_path}")
print(f"  Best checkpoint:       {args.checkpoint_dir}")
print(f"  Intermediate outputs:  {args.output_dir}")
print()
...
if usewandb:
    import wandb
    watermark = args.wandb_run_name if hasattr(args, 'wandb_run_name') else "{}_lr{}_pathmnist".format(args.net, args.lr)
    
    # Configure W&B to save logs in output_dir
    wandb_dir = os.path.join(args.output_dir, 'wandb')
    os.makedirs(wandb_dir, exist_ok=True)
    
    wandb.init(project=args.wandb_project if hasattr(args, 'wandb_project') else "PathMNIST-MLO-MAE",
            name=watermark,
            dir=wandb_dir)
    wandb.config.update(args)
    print(f"✓ W&B initialized - logs will be saved to: {wandb_dir}")
...
# Data
print('==> Preparing PathMNIST data..')
size = imsize
...
# Prepare dataset
trainset = PathMNIST(split='train', transform=transform_train, download=True, root=args.data_path)
valset = PathMNIST(split='val', transform=transform_test, download=True, root=args.data_path)
testset = PathMNIST(split='test', transform=transform_test, download=True, root=args.data_path)
...
class_names_short = [f'C{i}' for i in range(len(classes))]  # Short names for display
...
if args.use_pretrained:
    pretrain_path = os.path.join(args.pretrained_path, 'pretrain.pth')
...
if args.use_finetune:
    finetune_path = os.path.join(args.pretrained_path, 'finetune.pth')
...
if args.resume:
    ...
    # Try finetune_best.pth first, then fall back to old naming
    checkpoint_path = os.path.join(args.checkpoint_dir, 'finetune_best.pth')
...
def test(epoch):
    ...
    if acc > best_acc:
        print(f'Saving best model... (new best: {acc:.2f}% > previous: {best_acc:.2f}%)')
        state = {
            "net": net.state_dict(),
            "acc": acc,
            "epoch": epoch,
        ...
        # Save as finetune_best.pth (primary)
        best_checkpoint_path = os.path.join(args.checkpoint_dir, 'finetune_best.pth')
        torch.save(state, best_checkpoint_path)
        print(f'✓ Best model saved to: {best_checkpoint_path}')
        
        # Also save to output_dir for backup
        if args.output_dir != args.checkpoint_dir:
            backup_path = os.path.join(args.output_dir, f'epoch_{epoch}_acc_{acc:.2f}.pth')
...
print(f'✓ Best model saved to: {os.path.join(args.checkpoint_dir, "finetune_best.pth")}')

# Save final metrics
import json
final_metrics = {
    'best_accuracy': float(best_acc),
    'final_epoch': args.n_epochs - 1,
    'model': args.net,
    'dataset': 'PathMNIST',
