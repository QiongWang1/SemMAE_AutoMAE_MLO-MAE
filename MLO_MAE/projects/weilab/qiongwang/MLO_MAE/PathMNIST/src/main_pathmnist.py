...
# Add MLO_MAE root directory to path to import shared modules
...
def save_checkpoint(epoch, model, optimizer, output_directory="./checkpoint_pathmnist", filename="checkpoint.pth.tar"):
...
path_save = './checkpoint_pathmnist'
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./output_pathmnist',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./log_pathmnist',
    ...
    # additional setting
    parser.add_argument('--wandb_project', type=str, default='MLO-MAE-PathMNIST')
    parser.add_argument('--wandb_run_name', type=str, default='pathmnist_mlo_pretrain')
...
def main(args):
    ...
    # PathMNIST normalization - use ImageNet normalization for better transfer learning
    ...
    train_data = PathMNIST(split='train', transform=train_transform, download=True, root=args.data_path)
    valid_data = PathMNIST(split='val', transform=valid_transform, download=True, root=args.data_path)
    test_data = PathMNIST(split='test', transform=valid_transform, download=True, root=args.data_path)
    
    # Get number of classes from PathMNIST INFO
    info = INFO['pathmnist']
    num_classes = len(info['label'])
    logging.info(f"PathMNIST - {num_classes} classes")
    
    # Combine train and val for MLO split
    combined_train = torch.utils.data.ConcatDataset([train_data, valid_data])
    num_train = len(combined_train)
    indices = list(range(num_train))
    split = int(np.floor(args.train_portion * num_train))

    logging.info(f"Total training samples: {num_train}")
    logging.info(f"Split for pretraining: {split}, validation for mask: {num_train - split}")
...
#        drop_last=True,
    ...
    finetune_module = models_vit_mlo_ddp.FinetuneVisionTransformer(num_classes=num_classes).to(device)
    ...
    def log_reconstruction_images(model, masking_module, data_loader, device, epoch, args, num_images=8):
        ...
        if len(labels.shape) > 1:
            labels = labels[:num_images].squeeze()
        else:
        ...
        with torch.no_grad():
            ...
            x, mask, ids_restore, mask_prob = masking_module(x, args.mask_ratio, random=False)
            
            # Get predictions
            pred = model.forward(x, mask, ids_restore)
            ...
            pred_img_vis = pred_img * std + mean
            
            images_vis = torch.clamp(images_vis, 0, 1)
            pred_img_vis = torch.clamp(pred_img_vis, 0, 1)
            ...
            for idx in range(num_images):
                # Original
                img_orig = images_vis[idx].cpu().permute(1, 2, 0).numpy()
                axes[idx, 0].imshow(img_orig)
                axes[idx, 0].set_title(f'Original (Class {labels[idx].item()})', fontsize=10)
                axes[idx, 0].axis('off')
                
                # Masked (show which patches are masked)
                img_masked = img_orig.copy()
                mask_np = mask[idx].cpu().numpy()
                patch_size = model.patch_embed.patch_size[0]
                h = w = int(mask_np.shape[0] ** 0.5)
                mask_2d = mask_np.reshape(h, w)
                mask_img = np.kron(mask_2d, np.ones((patch_size, patch_size)))
                for c in range(3):
                    img_masked[:, :, c] = img_masked[:, :, c] * (1 - mask_img * 0.7)
                axes[idx, 1].imshow(img_masked)
                axes[idx, 1].set_title(f'Masked (75%)', fontsize=10)
                axes[idx, 1].axis('off')
                
                # Reconstructed
                img_recon = pred_img_vis[idx].cpu().permute(1, 2, 0).numpy()
                axes[idx, 2].imshow(img_recon)
                axes[idx, 2].set_title('Reconstructed', fontsize=10)
                axes[idx, 2].axis('off')
            
            plt.suptitle(f'MAE Reconstruction - Epoch {epoch}', fontsize=14)
            plt.tight_layout()
    ...
    class MaskEngine(Engine):
        ...
        def validation(self):
            ...
            # Log reconstruction images every 10 validation steps
            if hasattr(self, 'val_count'):
                self.val_count += 1
            else:
                self.val_count = 0
            
            if self.val_count % 10 == 0 and args.wandb_mode != 'disabled':
                try:
                    log_reconstruction_images(
                        self.pretrain.module, 
                        self.mask.module, 
                        test_dataloader, 
                        device, 
                        self.val_count // args.valid_step,
                        args,
                        num_images=6
                    )
                except Exception as e:
                    print(f"Warning: Could not log reconstruction images: {e}")
            ...
            if not args.baseline:
                save_checkpoint(None, model, None, output_directory=path_save, filename="pretrain_latest.pth")
                save_checkpoint(None, finetune_module, None, output_directory=path_save, filename="finetune_latest.pth")
                save_checkpoint(None, masking_module, None, output_directory=path_save, filename="masking_latest.pth")
                
                # Save BEST checkpoint if accuracy improved
                if best_acc < acc:
                    best_acc = acc
                    print(f'🎉 NEW BEST ACCURACY: {acc:.2f}% (previous: {best_acc:.2f}%)')
                    save_checkpoint(None, model, None, output_directory=path_save, filename="pretrain_best.pth")
                    save_checkpoint(None, finetune_module, None, output_directory=path_save, filename="finetune_best.pth")
                    save_checkpoint(None, masking_module, None, output_directory=path_save, filename="masking_best.pth")
                    # Also save to main checkpoint directory
                    save_checkpoint(None, model, None, output_directory="./checkpoint_pathmnist", filename="pretrain.pth")
                    save_checkpoint(None, finetune_module, None, output_directory="./checkpoint_pathmnist", filename="finetune.pth")
                    save_checkpoint(None, masking_module, None, output_directory="./checkpoint_pathmnist", filename="masking.pth")
            else:
                save_checkpoint(None, model, None, output_directory=path_save, filename="pretrain_baseline.pth")
            
            print('acc:', acc, 'best_acc:', best_acc)
            wandb.log({'test_acc': acc, 'best_test_acc': best_acc})
