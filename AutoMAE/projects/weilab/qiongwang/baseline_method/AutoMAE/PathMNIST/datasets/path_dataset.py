"""
PathMNIST Dataset Loader for AutoMAE
Loads MedMNIST PathMNIST dataset and provides PyTorch dataset interface
"""
...
class PathMNISTDataset(Dataset):
    """
    PathMNIST Dataset wrapper for AutoMAE pretraining
    ...
    def __init__(
        ...
        print(f"Loaded PathMNIST {split} split:")
...
def build_pathmnist_dataset(split='train', input_size=32, is_train=True, data_dir='./data', no_augmentation=False):
    """
    Build PathMNIST dataset with appropriate transforms for AutoMAE
    ...
    Returns:
        PathMNIST dataset instance
    """
    
    if is_train and not no_augmentation:
        # Training augmentations - SAFE transforms only
        # For medical images, geometric augmentations are more appropriate
...
def get_pathmnist_dataloaders(
    ...
    """
    Get train, validation, and test dataloaders for PathMNIST
    ...
    # Build datasets
    train_dataset = build_pathmnist_dataset('train', input_size, is_train=True, data_dir=data_dir, no_augmentation=no_augmentation)
    val_dataset = build_pathmnist_dataset('val', input_size, is_train=False, data_dir=data_dir, no_augmentation=no_augmentation)
    test_dataset = build_pathmnist_dataset('test', input_size, is_train=False, data_dir=data_dir, no_augmentation=no_augmentation)
...
class PathMNISTDataPrefetcher:
    """
    Data prefetcher for PathMNIST that normalizes images on GPU
    Similar to the HDF5 ImageNet prefetcher but adapted for PathMNIST
    """
...
def fast_collate_pathmnist(batch, memory_format=torch.contiguous_format):
    """
    Fast collate function for PathMNIST
