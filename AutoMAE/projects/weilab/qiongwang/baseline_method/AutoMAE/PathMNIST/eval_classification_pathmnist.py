"""
Classification Evaluation Script for PathMNIST
Fine-tunes pretrained AutoMAE encoder for 9-class classification
"""
...
from datasets.path_dataset import PathMNISTDataset, get_pathmnist_dataloaders
import models_mae_pathmnist as models_mae
import util.misc as misc


class ClassificationHead(nn.Module):
    """Classification head for PathMNIST"""
    def __init__(self, embed_dim=192, num_classes=9):
        super().__init__()
...
class MAEClassifier(nn.Module):
    ...
    def __init__(self, mae_model, num_classes=9):
        super().__init__()
...
def get_args_parser():
    parser = argparse.ArgumentParser('PathMNIST Classification Evaluation', add_help=False)
...
def save_results(results, output_dir):
    ...
    # Save to markdown
    md_path = os.path.join(output_dir, 'AutoMAE_PathMNIST_Results.md')
    with open(md_path, 'w') as f:
        f.write('# AutoMAE PathMNIST Classification Results\n\n')
        ...
        for i in range(9):
            f.write(f'| {i} | {results["precision_per_class"][i]*100:.2f}% | '
                   ...
                   f'{int(results["support_per_class"][i])} |\n')
    ...
    with open(csv_path, 'w') as f:
        ...
        for i in range(9):
            ...
            f.write(f'Class_{i}_Support,{int(results["support_per_class"][i])}\n')
...
def main(args):
    print('='*80)
    print('PathMNIST Classification with AutoMAE')
    print('='*80)
    ...
    try:
        ...
        print(f"\n✗ Error loading PathMNIST dataset: {e}")
